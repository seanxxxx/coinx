...
class TestAskEatBidCancelBidTrade(unittest.TestCase):
    """模拟深度->挂Ask->吃Bid（Bid一价部分交易）->bid一价撤单测试"""
    ...
    def askBidCancelBid(self):
        """tester01申报ask，吃掉test02申报的bid一价一部分"""
        ask_price_list = []
        bid_price_list = []
        ask_count_list = []
        bid_count_list = []
        ...
        print("\n")
        print("Ask申报 价格:%f 数量:%f"%(deal_price,deal_count))
        ...
        tester01_available_BTC = round(100 - ask_count_amount - deal_count,3)
        ...
        tester02_available_USDT = round(1000000 - bid_amount + (bid_first_price - deal_price) * deal_count,5)

        tester02_reserved_USDT = round(bid_amount - bid_first_price * deal_count,5)
        ...
        # 获取需要撤单的orderid
        search_orderid_sql = "select id from trade_order where price = %f and count = %f and status != 9000" % (bid_5_price,bid_5_count)
        cancel_orderid_data = ctcdbUtil.execute(search_orderid_sql)
        cancel_orderid = cancel_orderid_data[0][0]
        print("\n")
        print("撤单的id：", cancel_orderid)

        cancel_req = robot_fun.cancelOrder(get_basic_cfg.cancelOrderUrl,cancel_orderid,"订单备注",tester02_token)
        ...
        # 再次验证tester02的USDT
        tester02_USDT = ctcdbUtil.execute(get_ctcdb.search_tester02_money[1])
        tester02_available_USDT = round(tester02_available_USDT + bid_5_price * (bid_5_count - deal_count),8)
        tester02_reserved_USDT = round(tester02_reserved_USDT - bid_5_price * (bid_5_count - deal_count),8)
        ...
        print("3.用户2部分成交的bid挂单进行撤单")
        print("4.验证用户1第二次挂单的订单状态")
        print("5.验证用户2撤单的订单状态")
