...
class TestAskEatBidCancelAskTrade(unittest.TestCase):
    """模拟深度->挂Ask->吃Bid（Bid一价全部交易）->剩余Ask撤单测试"""
    ...
    def askBidCancelAsk(self):
        """tester01申报ask，吃掉test02申报的全部bid一价，剩余部分进入深度图后撤单"""
        ask_price_list = []
        bid_price_list = []
        ask_count_list = []
        bid_count_list = []
        ...
        #挂ask并吃bid一价的一部分（价格小于bid一价，大于bid二价，数量大于bid一价数量）
        ...
        next_ask_req = robot_fun.takeOrder('sell',
                                           deal_price,
                                           before_deal_count,
                                           get_basic_cfg.orderUrl,
                                           tester01_token)
        ...
        tester01_available_BTC = round(100 - ask_count_amount - before_deal_count,3)
        ...
        tester02_available_USDT = round(1000000 - bid_amount + (bid_5_price - deal_price) * deal_count,5)

        tester02_reserved_USDT = round(bid_amount - bid_5_price * deal_count,5)
        ...
        # 获取需要撤单的orderid
        userId = next_ask_req.json()['data']['id']
        cancel_orderid = robot_fun.getTradeOrderId(userId)
        ...
        sqlstr = "select status from trade_order where id = %s" % cancel_orderid
        data = ctcdbUtil.execute(sqlstr)
        ...
        # 再次验证tester01的BTC
        tester01_BTC = ctcdbUtil.execute(get_ctcdb.search_tester01_money[0])
        tester01_available_BTC = round(tester01_available_BTC + after_deal_count, 3)
        tester01_reserved_BTC = round(tester01_reserved_BTC - after_deal_count, 3)
