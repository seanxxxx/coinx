...
class TestAskEatBidAllTrade(unittest.TestCase):
    """模拟深度->挂Ask->吃Bid（Bid一价全部交易）测试"""
    ...
    def takeAll(self):
        """tester01申报ask，吃掉test02申报的全部bid一价"""
        ask_price_list = []
        bid_price_list = []
        ask_count_list = []
        bid_count_list = []
        ...
        #挂ask并吃bid一价的全部（价格小于bid一价，数量等于bid一价数量）
        deal_price = round(random.uniform(6900, bid_5_price), 2)
        ...
        print("2.用户1挂一个ask（价格小于bid一价，数量等于bid一价数量），吃掉全部bid一价")
