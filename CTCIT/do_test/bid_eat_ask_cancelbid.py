...
class TestBidEatAskCancelBidTrade(unittest.TestCase):
    ...
    def bidAskCancelBid(self):
        ...
        # 再次验证tester02的USDT
        tester02_USDT = ctcdbUtil.execute(get_ctcdb.search_tester02_money[1])
        tester02_available_USDT = round(tester02_available_USDT + deal_price * after_deal_count, 8)
        tester02_reserved_USDT = round(tester02_reserved_USDT - deal_price * after_deal_count, 8)
        ...
        print("2.用户2挂一个bid（价格大于ask一价，小于ask二价，数量大于ask一价数量），吃掉全部bid一价，剩余部分上架")
        print("3.用户2第二次挂单的剩余部分撤单")
        print("4.验证撤单的订单状态")
