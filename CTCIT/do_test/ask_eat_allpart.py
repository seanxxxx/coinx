...
class TestAskEatBidAllPartTrade(unittest.TestCase):
    """模拟深度->挂Ask->吃Bid（Bid一价全部交易，Bid二价部分交易）测试"""
    ...
    def takeAllPart(self):
        """tester01申报ask，吃掉test02申报的bid一价全部，二价部分"""
        ask_price_list = []
        bid_price_list = []
        ask_count_list = []
        bid_count_list = []
        ...
        #挂ask并吃bid一价的全部，二价的部分（价格小于bid二价，数量大于bid一价，小于bid一价和bid二价数量和）
        deal_price = round(random.uniform(6900, bid_4_price), 2)
        deal_count = round(random.uniform(bid_5_count, bid_4_count + bid_5_count), 3)
        ...
        tester02_available_USDT = round(1000000 - bid_amount + (bid_5_price - deal_price) * bid_5_count + (bid_4_price - deal_price) * (deal_count - bid_5_count),6)

        tester02_reserved_USDT = round(bid_amount - bid_5_price * bid_5_count - bid_4_price * (deal_count - bid_5_count),6)
        ...
        print("2.用户1挂一个ask（价格小于bid二价，数量大于bid一价，小于bid一价和bid二价数量和），吃掉全部bid一价，部分bid二价")
        print("3.验证第二次挂ask的订单状态")
