...
class TestBidEatAskAllPartTrade(unittest.TestCase):
    """模拟深度->挂Bid->吃Ask（Ask一价全部交易，Ask二价部分交易）测试"""
    ...
    def takeAllPart(self):
        ...
        deal_count = round(random.uniform(ask_1_count, ask_1_count+ask_2_count), 3)
        ...
        #交易总价
        deal_amount = round((ask_1_price * ask_1_count) + (ask_2_price * (deal_count - ask_1_count)),5)
        print("\n")
        ...
        tester02_available_USDT = round(1000000 - bid_amount - deal_price * deal_count + (deal_price - ask_1_price) * ask_1_count + (deal_price - ask_2_price) * (deal_count - ask_1_count),5)
        ...
        print("2.用户2挂一个bid（价格大于ask二价，数量大于ask一价，小于ask一价和ask二价数量和），吃掉全部ask一价，部分ask二价")
