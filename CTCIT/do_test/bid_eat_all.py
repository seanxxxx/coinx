...
class TestBidEatAskAllTrade(unittest.TestCase):
    """模拟深度->挂Bid->吃Ask（Ask一价全部交易）测试"""
    ...
    def takeAll(self):
        ...
        #挂bid并吃ask一价的一部分（价格大于ask一价，数量等于ask一价数量）
        deal_price = round(random.uniform(ask_1_price, 7100), 2)
        deal_count = ask_1_count
        ...
        print("2.用户2挂一个bid（价格大于ask一价，数量等于ask一价数量），吃掉全部ask一价")
