...
token01 = bch_usdt.robot_fun.get_access_token(url_login, tester01, password1)
token02 = bch_usdt.robot_fun.get_access_token(url_login, tester02, password1)

tradePairCode = 'bch_usdt'

while 1:
    try:
        bch_usdt_lastprice = float(bch_usdt.robot_fun.getMarketPrice(bch_usdt_url))
    ...
    for i in range(1,1000000):
        ...
        req1 = bch_usdt.robot_fun.order_buy(tester01, bch_usdt_lastprice, url_order, token01, tradePairCode, count1)
        #设置等待时间
        ...
        req2 = bch_usdt.robot_fun.order_sell(tester02, bch_usdt_lastprice, url_order, token02, tradePairCode, count2)
        #设置等待时间
        if round(nowTime) % random.randint(1, i + 2):
            time.sleep(random.uniform(0, 5))
        else:
        ...
        if req1.json()['msg'] == 'AccessToken已过期':
            bch_usdt.robot_fun.get_access_token(url_login, tester01, password1)

        if req2.json()['msg'] == 'AccessToken已过期':
            bch_usdt.robot_fun.get_access_token(url_login, tester02, password2)
