...
token01 = ltc_btc.robot_fun.get_access_token(url_login, tester01, password1)
token02 = ltc_btc.robot_fun.get_access_token(url_login, tester02, password1)

tradePairCode = 'ltc_btc'

while 1:
    try:
        ltc_btc_lastprice = float(ltc_btc.robot_fun.getMarketPrice(ltc_btc_url))
    ...
    for i in range(1,1000000):
        ...
        req1 = ltc_btc.robot_fun.order_buy(tester01, ltc_btc_lastprice, url_order, token01, tradePairCode, count1)
        #设置等待时间
        ...
        req2 = ltc_btc.robot_fun.order_sell(tester02, ltc_btc_lastprice, url_order, token02, tradePairCode, count2)
        #设置等待时间
        if round(nowTime) % random.randint(1, i + 2):
            time.sleep(random.uniform(0, 5))
        else:
        ...
        if req1.json()['msg'] == 'AccessToken已过期':
            ltc_btc.robot_fun.get_access_token(url_login, tester01, password1)

        if req2.json()['msg'] == 'AccessToken已过期':
            ltc_btc.robot_fun.get_access_token(url_login, tester02, password2)
