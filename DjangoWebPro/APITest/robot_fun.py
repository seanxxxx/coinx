...
#挂买单
def order_buy(email,price,url_order,access_token,tradePairCode):
...
#挂卖单
def order_sell(email,price,url_order,access_token,tradePairCode):
    price = getPrice(price)
    count = 0
    # if tradePairCode == 'btc_usdt':
    #     count = round(random.uniform(0.001, data['tradePair']['btc_usdtMaxCount']), 3)
