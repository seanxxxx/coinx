...
while 1:
    ...
    for i in range(1,1000000):
        count1 = round(random.uniform(0.001, data['tradePair']['xrp_btcMaxCount']), 0)
        req1 = robot_fun.order_buy(tester01, xrp_btc_lastprice, url_order, token01, tradePairCode, count1)
        #设置等待时间
        ...
        count2 = round(random.uniform(0.001, data['tradePair']['xrp_btcMaxCount']), 0)
        req2 = robot_fun.order_sell(tester02, xrp_btc_lastprice, url_order, token02, tradePairCode, count2)
        #设置等待时间
        if round(nowTime) % random.randint(1, i + 2):
            time.sleep(random.uniform(0, 5))
        else:
