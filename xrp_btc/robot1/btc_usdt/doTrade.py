...
robot07 = data['count']['robot07']
password1 = data['count']['password1']

robot08 = data['count']['robot08']
password2 = data['count']['password2']

token01 = robot_fun.get_access_token(url_login, robot07, password1)
token02 = robot_fun.get_access_token(url_login, robot08, password1)
...
while 1:
    ...
    for i in range(1,1000000):
        ...
        req1 = robot_fun.order_buy(robot07, btc_usdt_lastprice, url_order, token01, tradePairCode, count1)
        #设置等待时间
        ...
        req2 = robot_fun.order_sell(robot08, btc_usdt_lastprice, url_order, token02, tradePairCode, count2)
        #设置等待时间
        if round(nowTime) % random.randint(1, i + 2):
            time.sleep(random.uniform(0, 5))
        else:
        ...
        if req1.json()['msg'] == u'AccessToken已过期':
            robot_fun.get_access_token(url_login, robot07, password1)

        if req2.json()['msg'] == u'AccessToken已过期':
            robot_fun.get_access_token(url_login, robot08, password2)
